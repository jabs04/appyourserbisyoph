...
class BookingController extends Controller
{
    ...
    public function getBookingDetail(Request $request){
        ...
        if($bookingpackage !== null){
            $bookingpackage->package_image = $bookingpackage->package->getFirstMedia('package_attachment')->getUrl();
        }
    ...
    public function bookingUpdate(Request $request)
    {
        ...
        if($data['status'] == 'completed' && $data['payment_status'] == 'pending_by_admin'){
            ...
            if($user->sp_neo_id)
            {
                $serviceProvider = $user->sp_neo_id;
                
                $neoProvider     = User::where('id',$serviceProvider)->first();
                ...
                //update wallet ni neo jabu
                $neoWalletJabu = Wallet::where('user_id', $user->sp_neo_id)->first();
                if($neoWalletJabu){
                    $neoWalletCurrent = $neoWalletJabu->amount;
                    $neoWalletJabu->amount = $neoWalletJabu->amount + $totalNeoComm;
                    $neoWalletJabu->update();
                    DB::table('earnings_history')->insert([
                        'user_id'    => $user->sp_neo_id,
                        'booking_id' => $paymentdata->booking_id,
                        'comm'       => $totalNeoComm,
                ...
                if($user->sp_upline_id)
                {
                    $neoProviderId = $user->sp_upline_id;
                    
                    $uplineNeo = User::where('id',$neoProviderId)->first();
                    ...
                    //update wallet ni upline jabu
                    $uplineWalletJabu = Wallet::where('user_id', $user->sp_upline_id)->first();
                    if($uplineWalletJabu){
                        $uplineCurrentWallet = $uplineWalletJabu->amount();
                        $uplineCurrentWallet->amount = $uplineCurrentWallet + $totalUplineComm;
                        $uplineWalletJabu->update();
                        DB::table('earnings_history')->insert([
                            'user_id'    => $user->sp_upline_id,
                            'booking_id' => $paymentdata->booking_id,
                            'comm'       => $totalUplineComm,
                            'created_at' => date('Y-m-d H:i:s')
                        ]);
            ...
            if($handyman->area)
            {
                //jabu add earninng depot wallet
                $depotWallet = Wallet::where('user_id', $handyman->area)->first();
                if($depotWallet){
                    $depoCurrentWallet = $depotWallet->amount;
                    $depotWallet->amount = $depoCurrentWallet + $totalCityManagerComm;
                    $depotWallet->update();
                }
